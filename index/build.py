...
def _build_tfidf(cfg: Dict, docs: List[Dict], outdir: Path, force: bool) -> None:
    if _tfidf_exists(outdir) and not force:
        print(f"[index.build] TF-IDF already present at {outdir} — skipping.")
        return
    ...
    idf_attr = getattr(vec, "idf_", None)
    ...
    (outdir / "tfidf_vocab.json").write_text(
        json.dumps({"vocabulary": vocab_out, "idf": idf_out}, ensure_ascii=True),
        encoding="utf-8",
    )
    (outdir / "tfidf_id_map.json").write_text(
        json.dumps(id_map_out, ensure_ascii=True), encoding="utf-8"
    )
...
def _build_whoosh(cfg: Dict, docs: List[Dict], force: bool) -> None:
    ...
    if _whoosh_exists(wdir) and not force:
        print(f"[index.build] Whoosh index already present at {wdir} — skipping.")
        return
...
def _build_faiss(cfg: Dict, npz_path: Path | None, outdir: Path, force: bool) -> None:
    ...
    if _faiss_exists(outdir) and not force:
        print(f"[index.build] FAISS index already present at {outdir} — skipping.")
        return

    data = np.load(npz_path, allow_pickle=True)
    emb = data["embeddings"].astype(np.float32)
    emb /= (np.linalg.norm(emb, axis=1, keepdims=True) + 1e-12)  # cosine/IP
    d = emb.shape[1]
