# Python 3.9 compatible
from __future__ import annotations
import json, re
from pathlib import Path
from typing import Dict, List, Tuple, Optional, Union

import numpy as np
from scipy.sparse import csr_matrix
from sklearn.metrics.pairwise import cosine_similarity

# ---- Optional Whoosh
try:
    from whoosh.index import open_dir as whoosh_open_dir
    from whoosh.qparser import MultifieldParser, OrGroup
    HAVE_WHOOSH = True
except Exception:
    HAVE_WHOOSH = False


# ------------------------ Index container ------------------------
class Indices:
    def __init__(
        self,
        *,
        tfidf_X: csr_matrix,
        vocab: Dict[str, int],
        row_id_map: List[str],
        whoosh_dir: Optional[Path] = None,
    ):
        self.tfidf_X = tfidf_X
        self.vocab = vocab
        self.row_id_map = row_id_map                 # row -> doc_id
        self.id2row = {d: i for i, d in enumerate(row_id_map)}

        self._windex = None
        if HAVE_WHOOSH and whoosh_dir and whoosh_dir.exists():
            try:
                self._windex = whoosh_open_dir(str(whoosh_dir))
            except Exception:
                self._windex = None


# ------------------------ Load TF-IDF ------------------------
def load_tfidf(outdir: Path) -> Tuple[csr_matrix, Dict[str, int], List[str]]:
    m = np.load(str(outdir / "tfidf_index.npz"))
    X = csr_matrix((m["data"], m["indices"], m["indptr"]), shape=tuple(m["shape"]))
    meta = json.loads((outdir / "tfidf_vocab.json").read_text(encoding="utf-8"))
    vocab = {str(k): int(v) for k, v in meta["vocabulary"].items()}

    # allow either id_map.json or tfidf_id_map.json
    p = outdir / "id_map.json"
    if not p.exists():
        p2 = outdir / "tfidf_id_map.json"
        if not p2.exists():
            raise FileNotFoundError("id map missing")
        p = p2
    row_id_map = json.loads(p.read_text(encoding="utf-8"))
    return X, vocab, row_id_map


# ------------------------ Query helpers ------------------------
_SPLIT_RE = re.compile(r"\s*//\s*")
_PUNCT_RE = re.compile(r"[^\w\s]+")

def _clean(s: str) -> str:
    s = s or ""
    s = _SPLIT_RE.split(s, maxsplit=1)[0].lower().strip()
    s = _PUNCT_RE.sub(" ", s)
    s = re.sub(r"\s+", " ", s).strip()
    return s

def _mk_queries(question: str, options: List[str], fanout: bool) -> List[str]:
    root = _clean(question)
    out = [root]
    if fanout:
        for o in options:
            oo = _clean(o)
            if oo and oo not in out:
                out.append(f"{root} {oo}")
    # tiny domain broadening
    if "immigr" in root:
        out.append(root + " united states america values opinions")
    return out


# ------------------------ Whoosh BM25 ------------------------
def _whoosh_search(windex, queries: List[str], hits_per_query: int) -> List[Tuple[str, float]]:
    if not (HAVE_WHOOSH and windex):
        return []
    res_all: List[Tuple[str, float]] = []
    with windex.searcher() as s:
        parser = MultifieldParser(
            ["title", "description", "post_content", "content"],
            schema=windex.schema,
            group=OrGroup,
        )
        for q in queries:
            try:
                qobj = parser.parse(q)
                res = s.search(qobj, limit=hits_per_query)
                for hit in res:
                    did = hit.get("id")
                    if did:
                        res_all.append((did, float(hit.score)))
            except Exception:
                continue
    return res_all


# ------------------------ TF-IDF cosine ------------------------
def _tfidf_rank(idx: Indices, queries: List[str], top_k: int) -> List[Tuple[str, float]]:
    if idx.tfidf_X.shape[0] == 0:
        return []
    def qvec(q: str) -> csr_matrix:
        toks = q.split()
        cols = [idx.vocab[t] for t in toks if t in idx.vocab]
        if not cols:
            return csr_matrix((1, idx.tfidf_X.shape[1]), dtype=np.float32)
        data = np.ones(len(cols), dtype=np.float32)
        rows = np.zeros(len(cols), dtype=np.int32)
        return csr_matrix((data, (rows, np.array(cols))), shape=(1, idx.tfidf_X.shape[1]), dtype=np.float32)

    scores = np.zeros(idx.tfidf_X.shape[0], dtype=np.float32)
    for q in queries:
        v = qvec(_clean(q))
        if v.nnz == 0:
            continue
        cs = cosine_similarity(v, idx.tfidf_X, dense_output=False)  # 1 x N
        scores += np.asarray(cs.todense()).ravel().astype(np.float32)

    if float(scores.max(initial=0.0)) <= 0.0:
        return []
    top = np.argpartition(-scores, min(top_k, len(scores)-1))[:top_k]
    top = top[np.argsort(-scores[top])]
    return [(idx.row_id_map[i], float(scores[i])) for i in top]


# ------------------------ Fusion / de-dup ------------------------
def _rrf_fuse(runs: List[List[Tuple[str, float]]], k: int = 60) -> List[Tuple[str, float]]:
    agg: Dict[str, float] = {}
    for run in runs:
        for rnk, (did, _) in enumerate(run):
            agg[did] = agg.get(did, 0.0) + 1.0 / (k + (rnk + 1))
    return sorted(agg.items(), key=lambda x: -x[1])

def _dedup_by_tfidf(idx: Indices, ranked_ids: List[str], cos_thresh: float, cap: Optional[int]) -> Tuple[List[str], int, int]:
    """Returns (kept_ids, uniques_in, removed_count)."""
    keep: List[str] = []
    seen_rows: List[int] = []
    uniques_in = 0
    removed = 0
    seen_set = set()
    for did in ranked_ids:
        if did in seen_set:
            continue
        seen_set.add(did)
        uniques_in += 1
        row = idx.id2row.get(did)
        if row is None:
            continue
        v = idx.tfidf_X[row]
        dup = False
        for r in seen_rows:
            if cosine_similarity(v, idx.tfidf_X[r])[0, 0] >= cos_thresh:
                dup = True
                break
        if dup:
            removed += 1
            continue
        keep.append(did)
        seen_rows.append(row)
        if cap and len(keep) >= cap:
            break
    return keep, uniques_in, removed


# ------------------------ Public API ------------------------
def build_indices(cfg: dict) -> Indices:
    outdir = Path(cfg["index"]["output_dir"])
    tfidf_X, vocab, row_id_map = load_tfidf(outdir)
    whoosh_dir = None
    if cfg.get("whoosh"):
        whoosh_dir = Path(cfg["whoosh"]["index_dir"])
    return Indices(tfidf_X=tfidf_X, vocab=vocab, row_id_map=row_id_map, whoosh_dir=whoosh_dir)

def retrieve_candidates(
    question: str,
    options: List[str],
    cfg: dict,
    idx: Indices,
) -> Union[List[str], Tuple[List[str], dict]]:
    """Returns either just ids (back-compat) or (ids, stats)."""
    fanout        = bool(cfg.get("retrieval", {}).get("fanout", True))
    hits_per_q    = int(cfg.get("whoosh", {}).get("hits_per_query", 600))
    top_k_sparse  = int(cfg.get("retrieval", {}).get("top_k_sparse", 600))
    min_cands     = int(cfg.get("retrieval", {}).get("min_candidates", 160))
    rrf_k         = int(cfg.get("retrieval", {}).get("rrf_k", 60))
    dedup_t       = float(cfg.get("filtering", {}).get("dedup_sim_thresh", 0.96))
    backend       = str(cfg.get("retrieval", {}).get("sparse_backend", "both"))  # "bm25"|"tfidf"|"both"

    queries = _mk_queries(question, options, fanout)

    runs: List[List[Tuple[str, float]]] = []
    bm25 = []
    tfidfr = []

    if backend in ("bm25", "both"):
        bm25 = _whoosh_search(idx._windex, queries, hits_per_q) if idx._windex else []
        if bm25: runs.append(bm25[:top_k_sparse])

    if backend in ("tfidf", "both"):
        tfidfr = _tfidf_rank(idx, queries, top_k_sparse)
        if tfidfr: runs.append(tfidfr[:top_k_sparse])

    # fallback to deterministic slice if nothing
    if all(len(r) == 0 for r in runs):
        ids = idx.row_id_map[:min_cands]
        stats = {
            "num_queries": len(queries),
            "bm25_hits": 0,
            "tfidf_hits": 0,
            "fused_size": 0,
            "uniques_in": 0,
            "removed_dups": 0,
            "dup_removed_pct": 0.0,
            "final_n": len(ids),
            "top_k_sparse": top_k_sparse,
            "dedup_sim_thresh": dedup_t,
        }
        return ids, stats

    fused = _rrf_fuse(runs, k=rrf_k)
    fused_ids = [did for did, _ in fused]

    deduped, uniques_in, removed = _dedup_by_tfidf(idx, fused_ids, cos_thresh=dedup_t, cap=top_k_sparse)

    # ensure minimum
    if len(deduped) < min_cands and tfidfr:
        for did, _ in tfidfr:
            if did not in deduped:
                deduped.append(did)
                if len(deduped) >= min_cands:
                    break

    final_cap = max(min_cands, top_k_sparse)
    ids = deduped[:final_cap]

    stats = {
        "num_queries": len(queries),
        "bm25_hits": len(bm25),
        "tfidf_hits": len(tfidfr),
        "fused_size": len(fused_ids),
        "uniques_in": uniques_in,
        "removed_dups": removed,
        "dup_removed_pct": (removed / float(max(1, uniques_in))) if uniques_in else 0.0,
        "final_n": len(ids),
        "top_k_sparse": top_k_sparse,
        "dedup_sim_thresh": dedup_t,
    }
    return ids, stats
